...
playtest.html の戦闘画面は絵文字ベース（`🤖`/`👹` 等）で画像素材はゼロ。
index.html には戦闘モーダル自体がまだ存在しない。

### 決定事項

**スプライト共有**
- ミニマップ用ドット絵（##2）を戦闘画面でも流用する
- 素材・パレット定義を一元管理し、両方の画面で参照する

**表示仕様**
- サイズ：2倍 or 3倍に拡大（戦闘UIのレイアウト実装時に決定）
- HP表現：テキスト表記（`HP 28/28`）＋ スプライトの色トーン切替（緑/黄/赤）
- レイアウト：playtest.html の2カラム構成を踏襲（左：敵、右：味方）
...
- ドット絵はコード内のピクセル配列として定義（画像ファイル不要）
- パレット3色（緑/黄/赤）の切替もコードで制御
