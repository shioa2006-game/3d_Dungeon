...
- `js/player.js` にプレイヤーステータスを追加
  - `player` オブジェクトに HP/ATK/REC/AGI/GOLD を追加（playtest.html の初期値・計算式を流用）
  - `playerStats()` / `playerAtkVs()` を実装
- `index.html` にバトル用 HTML 追加（3D ビュー領域に切替表示）
- `css/style.css` にバトル用スタイル追加
- `checkMonsterContact()` を `startBattle()` 呼び出しに接続
- キー操作：バトル中は3D移動キーをバトルコマンドに転用（↑↓でターゲット、←→でコマンド、Enterで実行）
- 戦闘中も「世界時間」は進む（ターン制の裏でAI動く）
- `battleLocked` フラグでバトル中の参加ユニット移動を停止
- ワールドターン中に新たなユニットが隣接エリアに来た場合、バトルに追加参戦する処理
- バトル中は 3D ビュー領域をバトル UI に差し替え（右パネル・ミニマップは維持）
- `M` キーによる全体マップはバトル中も使用可能

**完了条件**：敵に接触するとバトル画面が開く。自陣近くなら味方NPCが参戦。戦闘中もミニマップ・全体マップ（Mキー）でワールド状態を確認できる。戦闘終了後、元の3Dビューに戻る。

---

### Phase 7 — 自陣クリスタル機能

**目的**：自陣クリスタル上でQ=回復／E=ガチャを使える。

**作業内容**
- `js/shop.js` 作成
  - `SHOP_POOL` を tier 別確率テーブルに再定義（playtest の SHOP_POOL をベースに変換）
  - `openGacha`, `rollGacha` 実装（金額3段階：低額／中額／高額）
  - 当選時：現在の装備と差し替えるか保持するかを選択
- `healAtCrystal()` 移植
- プレイヤー装備スロット（weapon/armor/accessory）
- ガチャモーダルHTML/CSS追加
- 入力：クリスタル上でのみQ/E有効、ガチャ中は↑↓Enter/Esc

> ショップ形式（確定購入）からガチャ形式（確率抽選）に変更。詳細は `IMPL_NOTES.md ##9` 参照。

**完了条件**：自陣クリスタルに立って Q で回復、E でガチャを引き装備を入手できる。ステータスがサイドバーに反映される。
