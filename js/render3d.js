...
// ##18: exp() ルックアップテーブル
// drawFloor の per-pixel ループで毎フレーム数万回呼ばれる Math.exp を、
// 整数距離をキーにした事前計算テーブル参照に置換する（最大の per-pixel 削減）。
// 1500 を超えると exp(-d/160) は事実上 0 なので 2048 でクランプ。
const EXP_TABLE_SIZE    = 2048;
const EXP_TABLE_MAX_IDX = EXP_TABLE_SIZE - 1;
const _floorExpTable = new Float32Array(EXP_TABLE_SIZE);
const _ceilExpTable  = new Float32Array(EXP_TABLE_SIZE);
for (let i = 0; i < EXP_TABLE_SIZE; i++) {
  _floorExpTable[i] = Math.exp(-i / LIGHT_SCALE_FLOOR);
  _ceilExpTable[i]  = Math.exp(-i / LIGHT_SCALE_CEILING);
}
...
function drawFloor() {
  ...
  // ##18 ルックアップテーブル参照に変更（Math.exp は呼ばない）
  const fExp = _floorExpTable;
  ...
  for (let y = 1; y < fh; y++) {
    ...
    for (let x = 0; x < fw; x++) {
      ...
      // exp テーブル参照（インデックスは整数距離、上限クランプ）
      let _idx = actualDist | 0;
      if (_idx > EXP_TABLE_MAX_IDX) _idx = EXP_TABLE_MAX_IDX;
      const shade     = fExp[_idx];
      const ceilShade = cExp[_idx];
      const fogA      = 1 - shade;
...
function drawView3D(hits) {
  ...
  // ##18 壁シェーディングも exp テーブル参照（呼び出し回数少ないが統一）
  const wExp = _floorExpTable;
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    // ① 指数フォールオフ（テーブル参照） + ② 方向別シェーディング（縦壁=E/W面を暗く）
    let _wIdx = corrDist | 0;
    if (_wIdx > EXP_TABLE_MAX_IDX) _wIdx = EXP_TABLE_MAX_IDX;
    const distShade = wExp[_wIdx];
