...
**目的**：自陣クリスタル上でQ=回復／E=ショップ（装備購入）を使える。

**作業内容**
- `js/shop.js` 作成
  - `SHOP_POOL` 移植（playtest.html の SHOP_POOL をそのまま流用）
  - `openShop`, `buyItem` 実装（確定購入方式）
  - 購入時：既存装備は半値で自動買取、差し替えて装備
- `healAtCrystal()` 移植
- プレイヤー装備スロット（weapon/armor/accessory）
- ショップモーダルHTML/CSS追加
- 入力：クリスタル上でのみQ/E有効、ショップ中は↑↓Enter/Esc

**完了条件**：自陣クリスタルに立って Q で回復、E でショップを開き装備を購入できる。ステータスがサイドバーに反映される。
