...
**ラベル：[即時]** | 議論日：2026-04-20 | 起点：Phase 5実装時
...
**ラベル：未定** | 議論日： | 起点：Phase 5実装時

### 課題

3Dビューで複数ユニットが同一セルまたは隣接セルにいる場合、スプライトが重なって描画される。
現状は深度バッファ順に描画されるだけで、重なり方の制御がない。
「手前ユニットの背中／奥ユニットの正面」を自然に見せるか、完全に重ねるかなど、見た目の方針が未決定。
...
現状の左右キーによる視点回転は即時切替（90度ずつスナップ）。
ゆっくり方向転換するアニメーション（スムーズターン）にした場合、
操作感が変わるためゲームバランスや移動のテンポへの影響を検討する必要がある。
...
**ラベル：[即時]** | 議論日： | 起点：Phase 5実装時

### 課題

index.html には戦闘モーダルが存在しない。
playtest.html の `startBattle` / `renderBattle` / `battleAttack` / `battleFlee` / `endBattle` を移植する必要があるが、
3D版固有の状態管理（`battleLocked`・世界時間との並走・キー競合）との統合方針が未決定。
